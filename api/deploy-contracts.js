...
async function deployContracts() {
  try {
    ...
    const packageIdMatch = deployOutput.match(
      /Package published at:\s*(0x[a-fA-F0-9]+)/
    );
    if (!packageIdMatch) {
    ...
    if (adminCapMatch) {
      ...
      console.log(
        "Please find the AdminCap object ID in the deployment output above"
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  deployContracts()
    .then(() => {
      console.log(
        "🎊 All done! Update your .env file with the configuration above."
      );
    })
    .catch(console.error);
}
