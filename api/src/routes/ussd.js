...
import { getUser, registerUser, updateUser, getUserTransactions, addTransaction } from '../services/database.js';
 ...
 */
router.post('/', rateLimitUSSD, async (req, res) => {
  ...
  try {
    ...
    // =================================================================
      ...
      // 4. Process Registration
        ...
        if (pin !== confirmPin) {
          response = `END PINs do not match. Please restart the registration process.`;
        } else {
          ...
          response = `END Welcome, ${fullName}! Your SuiFlow wallet is created.\n` +
    ...
    // =================================================================
      ...
      // 2. Ask for Amount
        ...
        if (!receiver) {
          response = `END The recipient phone number is not registered with SuiFlow.`;
        } else if (receiverPhone === phoneNumber) {
            response = `END You cannot send SUI to yourself. Please enter a different recipient.`;
        } else {
      ...
      // 4. Process Transaction
        ...
        if (!isValidPin) {
          ...
          if (balance < amount) {
            response = `END Insufficient balance. You have ${formatSui(balance)}.`;
          } else {
            const receiver = await getUser(receiverPhone);
            ...
            sendSui(decryptedMnemonic, receiver.suiAddress, amount)
              .then(result => {
                ...
                console.log(`Transaction from ${phoneNumber} completed with status: ${result.status}`);
              })
              .catch(console.error);

            response = `END Transaction of ${formatSui(amount)} to ${receiver.fullName} is being processed.\n`+
    ...
    // =================================================================
      ...
      if (transactions.length === 0) {
        ...
        transactions.forEach(tx => {
          ...
          const counterparty = type === 'Sent' ? tx.receiverName : tx.senderName;
          txList += `${type} ${formatSui(tx.amount)} ${type === 'Sent' ? 'to' : 'from'} ${counterparty} - ${tx.status}\n`;
        });
        response = txList;
      }
    }
