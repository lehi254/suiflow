 ...
 */
router.post('/register', validateRequest(registerAdminSchema), async (req, res) => {
  try {
 ...
 */
router.get('/dashboard', verifyToken, requireRole([USER_ROLES.ADMIN]), async (req, res) => {
  try {
    ...
    const recentUsers = allUsers.filter(user => new Date(user.createdAt) > thirtyDaysAgo);
    ...
    const successfulTransactions = allTransactions.filter(tx => tx.status === 'success').length;
 ...
 */
router.get('/users', verifyToken, requireRole([USER_ROLES.ADMIN]), validateQuery(getUsersSchema), async (req, res) => {
  try {
 ...
 */
router.get('/transactions', verifyToken, requireRole([USER_ROLES.ADMIN]), validateQuery(getAllTransactionsSchema), async (req, res) => {
  try {
    const { limit, offset } = req.query;
    
    console.log(`🔄 Getting transactions with limit ${limit}, offset ${offset}`);
 ...
 */
router.get('/user/:phone', verifyToken, requireRole([USER_ROLES.ADMIN]), async (req, res) => {
  try {
    ...
    const { getUser, getUserTransactions } = await import('../services/database.js');
    ...
    res.json({
      ...
      data: {
        ...
        transactions: transactions.map(tx => ({
          ...
          counterparty: tx.senderPhone === phone ? {
 ...
 */
router.get('/system/health', verifyToken, requireRole([USER_ROLES.ADMIN]), async (req, res) => {
  try {
