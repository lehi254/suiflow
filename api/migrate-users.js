...
async function migrateUsers() {
  try {
    ...
    if (!process.env.CONTRACTS_PACKAGE_ID) {
      throw new Error('CONTRACTS_PACKAGE_ID not configured in .env file. Please deploy contracts first.');
    }
    ...
    for (const user of users) {
      try {
        ...
        if (user.walletObjectId) {
          console.log(`⏭️ User ${user.phone} already has smart contract wallet: ${user.walletObjectId}`);
        ...
        if (!fullUser || !fullUser.encryptedMnemonic) {
          console.error(`❌ Could not get encrypted mnemonic for user ${user.phone}`);
          errorCount++;
          continue;
        }
        ...
        console.log(`⚠️ User ${user.phone} needs to create smart contract wallet on next login`);
    ...
    console.log(`⏳ Pending migration: ${users.length - alreadyMigratedCount - migratedCount}`);
    
    if (users.length - alreadyMigratedCount - migratedCount > 0) {
      console.log('');
      console.log('ℹ️ Users without smart contract wallets will have them created on next login.');
    }
